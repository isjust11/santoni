...
export default function Slide3System({ data, onSelectVilla, onNavigate }: Slide3SystemProps) {
  // Let's determine custom grid classes for Bento Grid style asymmetry
  const getGridClasses = (id: string) => {
    switch (id) {
      case "santoni-1":
        return "md:col-span-4 lg:col-span-3 h-full";
      case "santoni-2":
        return "md:col-span-4 lg:col-span-3 h-full";
      case "santoni-3":
        return "md:col-span-8 lg:col-span-6 h-full";
      case "resort-santoni":
        return "md:col-span-12 lg:col-span-12 h-full";
      default:
        return "md:col-span-6 h-full";
    }
  };
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
          {data.map((item) => {
            ...
            const isResort = item.id === "resort-santoni";
              ...
              <div
                ...
                onClick={() => {
                  ...
                  if (item.id === "santoni-3") {
                    onNavigate(3); // Navigate to Slide 4 detail
                  } else {
                    onNavigate(8); // Or direct to Contact Form
                  }
                }}
                className={`group bento-card cursor-pointer flex flex-col justify-between overflow-hidden relative ${getGridClasses(
                  item.id
                )}`}
              >
                {/* Image Section */}
                <div className={`relative overflow-hidden w-full ${isResort ? "h-64 md:h-80" : "h-56 md:h-64"}`}>
                  <img
                    src={item.image}
                    alt={item.name}
                  ...
                  />
                  {/* Subtle glass gradients and badges */}
