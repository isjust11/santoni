...
export default function Slide6Amenities({ data }: Slide6AmenitiesProps) {
  // Dynamic icon helper to map string names from JSON to Lucide React components
  const renderIcon = (iconName: string) => {
    switch (iconName) {
      case "Waves":
        return <Icons.Waves className="w-8 h-8 text-brand-blue" />;
      case "Flame":
        return <Icons.Flame className="w-8 h-8 text-brand-blue" />;
      case "Utensils":
        return <Icons.Utensils className="w-8 h-8 text-brand-blue" />;
      case "Wifi":
        return <Icons.Wifi className="w-8 h-8 text-brand-blue" />;
      case "Car":
        return <Icons.Car className="w-8 h-8 text-brand-blue" />;
      case "Mic2":
        return <Icons.Mic2 className="w-8 h-8 text-brand-blue" />;
      default:
        return <Icons.Compass className="w-8 h-8 text-brand-blue" />;
    }
  };
  ...
  return (
    <section className="relative w-full min-h-screen py-20 px-6 md:px-12 lg:px-20 bg-brand-dominant flex flex-col justify-center overflow-hidden">
      {/* Decorative vertical lines representing contemporary architecture */}
      ...
      <div className="max-w-7xl mx-auto w-full z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((item, idx) => (
            <div
              key={idx}
              className="bento-card p-8 flex flex-col justify-between group cursor-default"
            >
              <div>
                {/* Icon wrapper with soft golden glow interaction */}
              ...
              {/* Decorative bottom gold outline indicator */}
