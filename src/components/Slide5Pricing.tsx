  ...
  return (
    <section className="relative w-full min-h-screen py-20 px-6 md:px-12 lg:px-20 bg-brand-dominant flex flex-col justify-center overflow-hidden">
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-stretch">
        ...
        {/* Left Side: Elegant cover banner matching Aman Resorts style (4 columns) */}
        <div className="lg:col-span-4 relative rounded-2xl overflow-hidden min-h-[300px] lg:min-h-full shadow-organic border border-brand-navy/5 flex flex-col justify-between p-8 group">
          ...
          />
          {/* Multi-layered dark overlay for text readability */}
          ...
          <div className="relative z-10 text-white">
            <span className="font-sans text-[10px] tracking-[0.2em] uppercase text-white/60 mb-2 block">
              04 / BIỂU GIÁ THƯƠNG HIỆU
            </span>
            <h3 className="font-serif text-2xl md:text-3xl font-bold mb-3 tracking-tight">
              Kỳ Nghỉ Trọn Vẹn Đang Chờ Bạn
            </h3>
            ...
            <button
              onClick={() => onNavigate(8)} // S9 Contact Form
              className="bg-white text-brand-blue hover:bg-brand-gold hover:text-brand-navy font-sans font-medium text-xs px-6 py-3 rounded-full transition-all duration-300 w-full"
            >
        ...
        {/* Right Side: Scrollable detailed prices & policies (8 columns) */}
        <div className="lg:col-span-8 flex flex-col justify-between">
          <div>
            <span className="text-brand-blue font-sans font-semibold text-xs tracking-[0.25em] uppercase mb-3 block">
              CÔNG KHAI & MINH BẠCH
            </span>
            <h2 className="font-serif text-3xl md:text-5xl text-brand-navy tracking-tight font-bold mb-3">
              {data.title}
            </h2>
            <p className="text-brand-navy/60 font-sans font-light text-sm md:text-base leading-relaxed mb-8 max-w-2xl">
              {data.subtitle}
            </p>
          </div>

          {/* Pricing Tables Container (Internal scrollable frame) */}
          <div className="space-y-6 max-h-[500px] overflow-y-auto pr-2">
            
            {/* 3 Categories: Weekday, Weekend, Holidays */}
            {data.categories.map((category, index) => (
              <div
                key={index}
                className="bg-white/70 backdrop-blur-sm rounded-xl border border-brand-navy/5 p-6 shadow-sm hover:border-brand-blue/25 transition duration-300"
              >
                  ...
                  <table className="w-full text-left border-collapse font-sans text-xs md:text-sm">
                    <thead>
                      <tr className="border-b border-brand-navy/10 text-brand-navy/50 uppercase tracking-wider text-[10px]">
                        ...
                        <th className="pb-3 font-medium text-right">Mức Ưu Đãi (Mùa du lịch)</th>
                      </tr>
                    </thead>
