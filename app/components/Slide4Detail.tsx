      ...
      <div className="max-w-7xl mx-auto w-full">
            ...
            {showVideo ? (
              <div className="absolute inset-0 bg-brand-navy flex flex-col items-center justify-center p-6 text-center text-white">
                ...
                <div className="relative z-10 flex flex-col items-center max-w-md">
                  ...
                  <button onClick={() => setShowVideo(false)} className="bg-white/10 hover:bg-white/20 border border-white/20 text-white font-sans text-xs px-6 py-2.5 rounded-full transition cursor-pointer">
                    Đóng Video
                  </button>
                </div>
              </div>
        ...
        <div className="bg-white/60 backdrop-blur-md rounded-2xl border border-brand-navy/5 shadow-organic p-6 md:p-10">
          <div className="flex flex-wrap items-center justify-center gap-2 border-b border-brand-navy/10 pb-6 mb-8">
            {(["specs", "amenities", "map", "faq"] as const).map((tab) => {
              const labels = { specs: "🏡 Sơ Đồ & Thông Số", amenities: "⭐ Tiện Ích Đẳng Cấp", map: "🗺️ Sơ Đồ Mặt Bằng", faq: "❓ FAQ Giải Đáp" };
              return (
                <button
                  key={tab}
                  id={`tab-${tab}`}
                  onClick={() => setActiveTab(tab)}
                  className={`px-5 py-2.5 rounded-full font-sans text-xs md:text-sm font-semibold uppercase tracking-wider cursor-pointer transition-all duration-300 ${activeTab === tab ? "bg-brand-blue text-white shadow-sm" : "bg-brand-navy/5 text-brand-navy/70 hover:bg-brand-navy/10"}`}
                >
                  {labels[tab]}
                </button>
              );
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center animate-fade-in">
                <div className="space-y-4">
                  ...
                  <ul className="space-y-3 font-sans text-sm text-brand-navy/80">
                    {[
                      ["Hạng phòng", data.specs.rooms],
                      ["Giường đệm", data.specs.beds],
                      ["Sức chứa tối đa", data.specs.capacity],
                      ["Diện tích khuôn viên", data.specs.size],
                      ["Không gian khách", data.specs.living],
                      ["Nhà bếp gia đình", data.specs.kitchen],
                    ].map(([label, value], i) => (
                ...
                <div className="bg-brand-blue/5 p-6 rounded-xl border border-brand-blue/10">
                  ...
                  <button onClick={() => onNavigate(8)} className="w-full bg-brand-blue hover:bg-brand-blue/90 text-white font-sans font-medium text-xs py-3 rounded-lg transition cursor-pointer">
            ...
            {activeTab === "map" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center animate-fade-in">
                <div className="space-y-4">
                  ...
                  <p className="font-sans text-sm text-brand-navy/80 leading-relaxed">Santoni 3 được quy hoạch thành 2 tầng vòm đối xứng hướng trực tiếp biển:</p>
                  <div className="space-y-3 font-sans text-sm">
                    <div className="p-3.5 bg-white rounded-lg border border-brand-navy/5 shadow-sm">
                      ...
                      <span className="text-brand-navy/70 leading-relaxed block">Phòng khách lớn đa năng, khu ăn uống ngoài trời, nhà bếp bếp đảo, 3 phòng ngủ khép kín, sảnh đón tiếp.</span>
                    </div>
                    <div className="p-3.5 bg-white rounded-lg border border-brand-navy/5 shadow-sm">
                      <span className="font-semibold text-brand-blue block mb-1">🔸 TẦNG LẦU 1 (First Floor)</span>
                      <span className="text-brand-navy/70 leading-relaxed block">5 phòng ngủ sang trọng khép kín, ban công ngắm biển, khu phòng tắm có bồn vòm cong kính lãng mạn, sân thượng BBQ ngoài trời.</span>
                    </div>
                  </div>
                </div>
                <div className="relative aspect-video bg-brand-navy/5 rounded-xl border border-brand-navy/10 flex flex-col items-center justify-center p-6 text-center text-brand-navy/50">
                  ...
                  <p className="font-sans text-xs max-w-xs leading-relaxed">Sơ đồ mặt bằng chi tiết của Santoni 3 được thiết kế bởi KTS Địa Trung Hải.</p>
                  <div className="absolute top-4 right-4 bg-brand-blue/10 px-2.5 py-1 rounded text-[10px] text-brand-blue tracking-widest uppercase font-semibold font-sans">CAD Drawing</div>
                </div>
              </div>
            )}
            ...
            {activeTab === "faq" && (
              <div className="space-y-3 animate-fade-in">
                ...
                {data.faq.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="border border-brand-navy/5 rounded-xl overflow-hidden transition-all duration-300">
                      <button
        ...
        {lightboxIndex !== null && (
          ...
          >
            <div className="absolute top-6 left-6 right-6 flex items-center justify-between text-white/80 z-20">
              ...
              <button onClick={() => setLightboxIndex(null)} className="p-2 hover:bg-white/10 rounded-full transition text-white hover:text-brand-gold cursor-pointer">
                <X className="w-6 h-6" />
              </button>
            </div>

            <div className="relative w-full max-w-5xl aspect-[16/10] md:aspect-[16/9] flex items-center justify-center z-10">
              <button onClick={prevImage} className="absolute left-4 p-3 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-white hover:text-brand-gold transition cursor-pointer z-30">
              ...
              />
              <button onClick={nextImage} className="absolute right-4 p-3 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-white hover:text-brand-gold transition cursor-pointer z-30">
            ...
            <div className="absolute bottom-6 left-6 right-6 text-center z-20">
              ...
              <p className="font-sans text-[10px] uppercase text-white/50 tracking-wider mt-2">Santoni Mũi Né Luxury Collection</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
